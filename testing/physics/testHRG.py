...
def testHRG():
    ...
    lpass *= print_results(chi_pdg, refPDG, prec=EPSILON, text="chiB2 PDG check")
    lpass *= print_results(chi_QM , refQM , prec=EPSILON, text="chiB2 QM check")
    lpass *= print_results(chi_ev , refEV , prec=EPSILON, text="chiB2 EV check")
    lpass *= print_results(chi_ev1, refEV1, prec=EPSILON, text="chiB2 EV1 check")
    ...
    lpass *= print_results(-chiusQM / (refQMBS + refQMQS) , np.ones(chiusQM.size)  , prec=EPSILON , text="chius QM check")
    lpass *= print_results(-chiusPDG / (refPDGBS + refPDGQS) , np.ones(chiusQM.size) , prec=EPSILON , text="chius PDG check")
    ...
    lpass *= print_results(ref3p_div_T4, test3p_div_T4, prec=2e-2, text="2014 HotQCD 3p/T^4 check")
    ...
    lpass *= print_results(refE_div_T4, testE_div_T4, prec=3e-2, text="2014 HotQCD e/T^4 check")
    ...
    lpass *= print_results(ref3S_div_4T3, test3s_div_4T3, prec=3e-2, text="2014 HotQCD 3s/4T^3 check")
    ...
    lpass *= print_results(ref3CV_div_T3, testCV_div_T3, prec=3e-2, text="2014 HotQCD CV/T^3 check")
...
#            pass
    ...
    lpass *= print_results(mesonP_div_T4, refMesonP_div_T4, prec=1.8e-1, text="2014 HotQCD meson open charm")
    ...
    lpass *= print_results(baryonP_div_T4, refBaryonP_div_T4, prec=2.2e-1, text="2014 HotQCD baryon open charm")
    ...
    baryonP_div_T4 = QMhrg.P_div_T4(refT,0,0,0)
    lpass *= print_results(baryonP_div_T4, refBaryonP_div_T4, prec=1e-3, text="2022 F. Karsch code open charm")
    ...
    lpass *= print_results(RSC13, refRSC13, prec=1.4e-1, text="2014 HotQCD RSC13")
    ...
    def compareAtmuB(muBh):
        ...
        for muSh in muBList:
            ...
            lpass2 *= print_results(left, right, prec=EPSILON, text="P vs gen_chi")
            ...
            lpass2 *= print_results(exact, numerical, prec=EPSILON, text="d(E/T^4)/dT")
            ...
            lpass2 *= print_results(exact, numerical, prec=EPSILON, text="d(P/T^4)/dT")
            ...
            lpass2 *= print_results(exact, numerical, prec=3*EPSILON, text="d(S/T^4)/dT")
            ...
            lpass2 *= print_results(exact, numerical, prec=EPSILON, text="d(chi11BQ)/dT")
            ...
            lpass2 *= print_results(exact, numerical, prec=EPSILON, text="d^2(chi11BQ)/dT^2")
    
            return lpass2
    
    lpass *= np.all( parallel_function_eval(compareAtmuB,muBList,nproc=DEFAULTTHREADS) )
    ...
    lpass *= print_results(exact, numerical, prec=1e-4, text="d(E/T^4)/dmuB")
    ...
    lpass *= print_results(exact, numerical, prec=1e-4, text="d(P/T^4)/dmuB")
    ...
    lpass *= print_results(exact, numerical, prec=1e-4, text="d(S/T^4)/dmuB")
