...
```
```Python
class EVHRG(Mass, g, w, B, S, Q, C=None):
'''
Excluded volume hadron resonance gas. b is excluded volume parameter. 
'''
```
```Python
class HRG(Mass, g, w, B, S, Q, C=None, NMAX_light=21, NMAX_heavy=2):
'''
...
```
```Python
class HRGbase(Mass, g, w, B, S, Q, C=None):
'''
...
```
```Python
class HRGexact(Mass, g, w, B, S, Q, C=None):
'''
